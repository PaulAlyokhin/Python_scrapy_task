...
def Print_Market_Profile(height_precision = 1, 
    ...
    cursor.execute("SELECT * FROM "+datatable_name)
    result = cursor.fetchall()
    upstox_frame = DataFrame(np.array(result).reshape(len(result),5), columns=['id','exchange', 'volume', 'time', 'Last_price'])
    upstox_frame = upstox_frame.loc[:,'time':'Last_price']
    temp_column = upstox_frame.loc[:,'Last_price':'Last_price']
    temp_column = temp_column.set_index('Last_price')
    ...
    time_groups = upstox_frame.groupby(pd.Grouper(freq=frequency))['Adj Close'].mean()
